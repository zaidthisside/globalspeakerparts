  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-in-out ${
        scrolled
          ? "bg-white/60 backdrop-blur-md border-b border-white/20 py-2 shadow-sm"
          : "bg-white/40 backdrop-blur-sm border-b border-white/10 py-4"
      }`}
    >
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12">
        <div className="flex items-center justify-between">
            ...
            {navigationItems.map((item) => {
              ...
              return (
                ...
                >
                  <Link 
                    href={item.href}
                    className={`px-1 py-1 rounded hover:text-accent-cyan transition-colors duration-150 ${
                      isActive ? "text-accent-cyan font-bold" : "text-heading-charcoal"
                    }`}
                  >
                    {item.name}
                  </Link>
                  ...
                  <AnimatePresence>
                    {isHovered && (
                      <motion.div
                        ...
                        className="absolute left-1/2 transform -translate-x-1/2 mt-2 w-60 glass-panel-solid rounded-lg shadow-xl overflow-hidden py-2 text-left z-50 text-[11px] font-sans normal-case tracking-normal"
                      >
                        <div className="px-3.5 py-1 text-[9px] font-bold text-accent-cyan uppercase tracking-widest border-b border-border-cool/40 mb-1">
                          {item.name} Options
                        </div>
                        {item.subItems.map((sub) => (
                          <Link
                            key={sub.name}
                            href={sub.href}
                            className="block px-4 py-2.5 text-primary-midnight hover:bg-white/60 hover:text-accent-cyan font-medium transition-colors border-b border-border-cool/10 last:border-0"
                          >
                            {sub.name}
                          </Link>
                        ))}
          ...
          <div className="flex items-center gap-1 sm:gap-2 shrink-0 z-10">
            ...
            >
              <Search className="w-5 h-5 text-primary-midnight hover:text-accent-cyan transition-colors" />
            </Link>

            {/* Cart Icon */}
            <Link 
              href="/products?cart=true"
              className="relative p-1.5 rounded-full hover:bg-white/25 transition-colors"
              aria-label="Cart"
            >
              <ShoppingBag className="w-5 h-5 text-primary-midnight hover:text-accent-cyan transition-colors" />
            </Link>

            {/* Quote button (desktop/tablet) */}
            <Link href="/contact?rfq=true" className="hidden sm:block">
              <button className="bg-primary-midnight text-white px-4 py-2.5 rounded-full font-semibold text-xs tracking-wider hover:bg-secondary-graphite transition-all uppercase cursor-pointer">
                Request Quote
              </button>
            </Link>
            ...
            <button
              ...
              className="lg:hidden p-2 text-primary-midnight hover:text-accent-cyan transition-colors cursor-pointer"
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-border-cool">
                ...
                <button
                  onClick={() => setIsOpen(false)}
                  className="p-2 text-primary-midnight hover:text-accent-cyan cursor-pointer"
              ...
              <nav className="flex-1 flex flex-col px-4 py-6 space-y-2.5 overflow-y-auto text-xs font-semibold uppercase tracking-wider scrollbar-thin">
                {navigationItems.map((item) => {
                  ...
                  return (
                    <div key={item.name} className="border-b border-border-cool/40 pb-2">
                      <button
                        onClick={() => setOpenMobileSub(isMobileSubOpen ? null : item.name)}
                        className="w-full flex items-center justify-between py-2 text-heading-charcoal hover:text-accent-cyan text-left font-bold"
                      >
                        <span>{item.name}</span>
                        <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${
                          isMobileSubOpen ? "transform rotate-180 text-accent-cyan" : ""
                        }`} />
                      </button>

                      {/* Sub-Items Accordion Loop strictly matching uploaded list */}
                      {isMobileSubOpen && (
                        <div className="pl-4 py-2 flex flex-col space-y-2 normal-case tracking-normal text-[11px] font-medium text-slate-650 bg-bg-snow/40 rounded-lg mt-1 border border-border-cool/20">
                          {item.subItems.map((sub) => (
                            <Link
                              key={sub.name}
                              href={sub.href}
                              onClick={() => setIsOpen(false)}
                              className="py-1.5 block hover:text-accent-cyan border-b border-border-cool/5 last:border-0"
                            >
                              {sub.name}
                            </Link>
                          ))}
              ...
              <div className="mt-auto px-6 py-6 border-t border-border-cool">
                <Link href="/contact?rfq=true" onClick={() => setIsOpen(false)}>
                  <button className="w-full bg-primary-midnight text-white py-3 rounded-full font-bold text-xs tracking-widest hover:bg-secondary-graphite transition-all cursor-pointer">
                    REQUEST B2B QUOTE
                  </button>
                </Link>
              </div>
