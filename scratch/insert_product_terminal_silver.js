...
async function main() {
  ...
  const srcName = 'media__1784965355343.jpg';
  const dstName = 'terminal-silver-red-black.jpg';
  ...
  const productName = "SPEAKER TERMINAL RED-BLACK WIRE IN SILVER";
  const productSlug = "speaker-terminal-red-black-wire-in-silver";
  ...
  const { data: productData, error: productError } = await supabase
    ...
    .insert({
      ...
      name: productName,
      slug: productSlug,
      short_desc: "Premium nickel/silver-plated speaker binding post terminals featuring red and black color-coded rings for secure, high-fidelity wire connections.",
      long_desc: "This pair of premium nickel/silver-plated speaker binding post terminals (one red, one black) is engineered for high-performance audio applications, custom speaker builds, and amplifier terminal replacements. The heavy-duty silver plating ensures maximum signal transfer and corrosion resistance, while the knurled thumb screws provide a secure grip for bare wire, spade lugs, or banana plugs.",
      featured_image: `/${dstName}`,
      is_hidden: false,
      is_featured: false,
      tags: ["speaker terminals", "binding posts", "silver plated", "speaker hardware", "audio connectors"],
      ...
      sort_order: 0,
      seo_meta: {
        title: "Silver-Plated Speaker Terminal binding Post Pairs | Global Speaker Parts",
        description: "Premium silver-plated red & black speaker terminals for high-performance audio connectivity.",
        hide_price: true
      }
    })
    .select()
  ...
  const { data: variantData, error: variantError } = await supabase
    ...
    .insert({
      ...
      name: "Silver Terminal Pair (Red/Black)",
      price: 1.70,
      stock: 3000,
      is_active: true,
      sort_order: 1,
      specs: {
        "Material": "Nickel/Silver-Plated Brass & Durable Polymer",
  ...
  console.log("Silver Speaker Terminal pair product listing fully complete!");
}
