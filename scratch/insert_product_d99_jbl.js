...
async function main() {
  ...
  const srcName = 'diaphragm_d99_jbl_clean_1784893289399.jpg';
  const dstName = 'diaphragm-d99-jbl.jpg';
  ...
  const productName = "SPEAKER DIAPHRAGM D-99 JBL";
  const productSlug = "speaker-diaphragm-d-99-jbl";
  ...
  const { data: productData, error: productError } = await supabase
    ...
    .insert({
      ...
      name: productName,
      slug: productSlug,
      short_desc: "Premium replacement speaker diaphragm featuring an etched titanium dome and round black mounting flange for D-99 JBL drivers.",
      long_desc: "This premium B2B replacement speaker diaphragm features a custom etched titanium dome and round black mounting flange tailored for D-99 JBL professional compression drivers. Engineered to meet strict performance tolerances, it provides clean, precise high-frequency response and reliable power handling. Ideal for professional sound reinforcement, touring speaker maintenance, and wholesale parts distribution.",
      featured_image: `/${dstName}`,
      is_hidden: false,
      is_featured: true,
      tags: ["diaphragms", "titanium dome", "compression drivers", "d-99-jbl", "speaker diaphragm"],
      applications: "Replacement diaphragm for D-99 JBL compression drivers, PA system speaker repair, stage monitor maintenance, and wholesale audio component distribution.",
      moq: "100 units",
      sort_order: 0,
      seo_meta: {
        title: "SPEAKER DIAPHRAGM D-99 JBL | Global Speaker Parts",
        description: "Premium replacement speaker diaphragm with etched titanium dome for D-99 JBL drivers."
      }
    })
    .select()
    .single();
  ...
  console.log("D-99 JBL Diaphragm product listing fully complete!");
}
